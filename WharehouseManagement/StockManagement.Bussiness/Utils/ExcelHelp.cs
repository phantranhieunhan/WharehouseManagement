...
using System.Xml;
using StockManagement.Business.Utils.Enum;
...
namespace StockManagement.Business.Utils.Enum
{
    public class ExcelHelper
    {
        ...
        public static List<T> LoadDataFromFile<T>(string inputPath, int fromRow, int fromColumn) where T : class
        {
                    ...
                    for (var rowIndex = fromRow; rowIndex <= workSheet.Dimension.End.Row; rowIndex++)
                    {
                        ...
                        foreach (var propertyInfo in objProp)
                        {
                            if (columnNames.Contains(propertyInfo.Name))
                            {
                                ...
                                {
                                    value = value == null || string.IsNullOrEmpty(value.ToString()) ? "" : value.ToString();
                                    propertyInfo.SetValue(objT, value);
                                    continue;
                                }
                            }
            ...
            {
                Logger.Error("ExcelHelper>LoadDataFromFile: " + ex);  
                return null;
            }
        }
            ...
            try
            {
                ...
                using (var package = new ExcelPackage(new FileInfo(outputPath)))
                {
                    ...
                    if (header != null)
                    {
                        ...
                        if (header.ContainsKey(Constant.ReportHeader.SITE))
                        {
                            workSheet.Cells[rowHeaderStart, 1].Value = header[Constant.ReportHeader.SITE]?.ToString() ?? "";
                            workSheet.Cells[rowHeaderStart, 1].Style.Font.Size = 14;
        ...
        public static bool ExportFromTemplate<T>(List<T> data, string xmlConfigPath, string outputPath, Dictionary<string, object> customData = null, List<string> columnDynamic = null, List<object> columnHeaderDynamic = null) where T : class
        {
            ...
            try
            {
                ...
                using (var package = new ExcelPackage(new FileInfo(outputTemplatePath)))
                {
                    ...
                    string sPosition = "", pIsSort = "", pColName = "", pAlign = "", pFormat = "", pValue = "";
                    int pPosition;
                    ...
                    for (int i = 0; i < contentRows; i++)
                    {
                        ...
                        foreach (XmlNode nCol in nContentColList)
                        {
                            ...
                            {
                                if (pValue != null)
                                    workSheet.Cells[currentRow, pPosition].Value = pValue;
                                else
                    ...
                    if (customData != null)
                    {
                        ...
                        {
                            var value = customData.ContainsKey(Constant.ReportHeader.STORE) ? (customData[Constant.ReportHeader.STORE]?.ToString() ?? "") : "";
                        ...
                        if (!string.IsNullOrEmpty(sPosition))
                        {
                            workSheet.Cells[sPosition].Value = customData.ContainsKey(Constant.ReportHeader.REPORT_DATE) ? (customData[Constant.ReportHeader.REPORT_DATE]?.ToString() ?? "") : "";
                        }
                    }
                    #endregion
        ...
        public static Dictionary<string, object> GetReportHeader(Dictionary<string, object> rptHeader)
        {
            try
            {
                bool isReportFromDate = false, isReportToDate = false;
                string reportDate = string.Empty, fromDate = string.Empty, toDate = string.Empty;
                var reval = new Dictionary<string, object>();
                ...
                foreach (var row in rptHeader)
                {
                    switch (row.Key.ToUpper())
                    {
                        case "FROMDATE":
                            {
                                isReportFromDate = true;
                                fromDate = ((DateTime)row.Value).ToString("dd/MM/yyyy");
                                break;
                            }
                        case "TODATE":
                            {
                                isReportToDate = true;
                                toDate = ((DateTime)row.Value).ToString("dd/MM/yyyy");
                                break;
                            }
                        case "APPNAME":
                            {
                                reval.Add(row.Key, "Ngành: " + row.Value);
                                break;
                            }
                        case "STORENAME":
                            {
                                reval.Add(row.Key, "Tên chi nhánh: " + row.Value);
                                break;
                            }
                        case "STOCKNAME":
                            {
                                reval.Add(row.Key, "Kho: " + row.Value);
                                break;
                            }
                        case "PRODUCTCATEGORY":
                            {
                                reval.Add(row.Key, "NHÓM HÀNG HÓA: " + row.Value);
                                break;
                            }
                        default:
                            {
                ...
                if (isReportFromDate && isReportToDate)
                {
                    reportDate = string.Format("Từ ngày {0}  Đến ngày {1}", fromDate, toDate);
                ...
                else
                {
                    if (isReportToDate)
                    {
                        reportDate = string.Format("Xem đến ngày {0}", toDate);
                        reval.Add("ReportDate", reportDate);
                    }
                }
                return reval;
        ...
        public static Dictionary<string, object> GetReportHeader(Dictionary<string, object> rptHeader, DateTime? fromDate, DateTime? toDate)
        {
            try
            {
                ...
                if (fromDate != null && toDate != null)
                    ...
                    reportDate = string.Format("Xem đến ngày {0}", ((DateTime)toDate).ToString("dd/MM/yyyy"));
                reval.Add("ReportDate", reportDate);
                ...
                foreach (var row in rptHeader)
                {
                    switch (row.Key.ToUpper())
                    {
                        case "STORECATEGORYNAME":
                            {
                                reval.Add(row.Key, "Nhóm: " + row.Value);
                                break;
                            }
                        ...
                        case "STORENAME":
                            {
                                reval.Add(row.Key, "Chi nhánh: " + row.Value);
                                break;
                            }
                        case "STOCKNAME":
                            {
                        ...
                        case "PRODUCTNAME":
                            {
                                reval.Add(row.Key, "Hàng hoá: " + row.Value);
                                break;
                            }
                        case "BARCODE":
                            {
                        ...
                        case "PRODUCTCODE":
                            {
                                reval.Add(row.Key, "Mã hàng: " + row.Value);
                                break;
                            }
                        case "UNITNAME":
                            {
                                reval.Add(row.Key, "ĐVT: " + row.Value);
                                break;
                            }
                        default:
                            {
